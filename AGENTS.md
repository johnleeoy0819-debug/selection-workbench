...
```
前端：Vercel — https://web-nu-eight-u4lwisjrqi.vercel.app
后端：EC2 18.234.173.5（FastAPI :8000）
通信：HTTP REST API，vercel.json 代理 /api/* → EC2
```
...
```
前端：React 19 + Vite 6 + TypeScript，minimalist-ui 风格
后端：Python 3.12+ / FastAPI / SQLite / Pydantic v2
...
- [x] 后端部署（EC2 :8000，MHH API v3.1.0）
- [x] 前端部署（Vercel，React + Vite）
- [x] 数据模型 + 数据库初始化
- [x] CSV 导入服务
- [x] 定价计算引擎
- [x] AI 评分引擎
